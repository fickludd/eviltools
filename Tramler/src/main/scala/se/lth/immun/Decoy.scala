...
class Decoy extends TramlOperation("DECOY") {
	...
	def setup(params:String, mods:Seq[IModifier]) = {
		...
		for (p <- params.split(","))
			p.trim match {
				...
				case x =>
					if (x.startsWith("decoy-factor=")) 
						decoyFactor = x.drop(13).toInt
					else if (x.startsWith("n=")) 
						nDecoys = x.drop(2).toInt
					else
						throw new IllegalArgumentException("Unknown param '"+x+"'")
			}
	}
	
	def usage:String = """Replace peptides and compounds with similar fakes. Options are
